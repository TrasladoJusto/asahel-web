...
export function SilkThread({ spiderY, spiderX, color, visible }: SilkThreadProps) {
  ...
  // Sincronizar refs tras el render (React 19: no escribir refs durante render)
  useEffect(() => {
  ...
  useEffect(() => {
    if (!visible) {
      ...
      // Reset de opacidad en el siguiente frame (evita setState síncrono en effect)
      const id = requestAnimationFrame(() => setOpacity(1));
      return () => cancelAnimationFrame(id);
    }
  }, [visible]);

  // Loop RAF completo dentro de un único effect (función local auto-recursiva)
  useEffect(() => {
    const drawFrame = () => {
      ...
      // Ancla
      if (anchorHalo) {
        anchorHalo.setAttribute('cx', String(anchorX));
        anchorHalo.setAttribute('cy', '0');
      }
      if (anchorDot) {
        anchorDot.setAttribute('cx', String(anchorX));
        anchorDot.setAttribute('cy', '0');
      }
      if (anchorGlow) {
        anchorGlow.setAttribute('cx', String(anchorX));
        anchorGlow.setAttribute('cy', '0');
      }
  ...
  return (
    ...
    >
      ...
      {/* 1. Sombra */}
      <path
        className="silk-path silk-shadow"
        d=""
        fill="none"
        stroke={color}
        strokeWidth="12"
      ...
      <path
        className="silk-path silk-fiber-1"
        d=""
        fill="none"
        stroke={color}
        strokeWidth="1.5"
      ...
      />
      <path
        className="silk-path silk-fiber-2"
        d=""
        fill="none"
        stroke={color}
        strokeWidth="1"
